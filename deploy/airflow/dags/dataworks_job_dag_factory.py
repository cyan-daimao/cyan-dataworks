...
DAG_DEFINITION_ENDPOINT = "/rpc/dataworks/airflow/job-dag-definitions"
LOG = logging.getLogger(__name__)


def _load_dag_definitions() -> list[dict]:
    url = f"{DATAWORKS_BASE_URL}{DAG_DEFINITION_ENDPOINT}"
    LOG.info("Loading DataWorks job DAG definitions from %s", url)
    try:
        response = requests.get(url, timeout=10)
        response.raise_for_status()
        payload = response.json()
    except Exception:
        LOG.exception("Failed to load DataWorks job DAG definitions from %s", url)
        return []
    definitions = payload.get("data") or []
    LOG.info("Loaded %s DataWorks job DAG definitions", len(definitions))
    return definitions
...
def _external_wait_task_id(dependency: dict, index: int) -> str:
    upstream_job_id = str(dependency.get("upstreamJobId") or "").strip()
    if upstream_job_id:
        return f"wait_for_job_{upstream_job_id}"
    upstream_dag_id = str(dependency.get("upstreamDagId") or "").strip()
    if upstream_dag_id:
        return f"wait_for_{upstream_dag_id}".replace("-", "_").replace(".", "_")
    return f"wait_for_external_job_{index + 1}"
...
for dag_def in _load_dag_definitions():
    try:
        ...
        job_id = dag_def["jobId"]
        task_id = "run_job"
        schedule = _normalize_cron_expression(dag_def.get("cronExpression"))
        LOG.info(
            "Registering DataWorks Job DAG: dag_id=%s, job_id=%s, raw_cron=%s, schedule=%s",
            dag_id,
            job_id,
        ...
        with DAG(
            ...
            for external_wait in external_wait_tasks:
                external_wait >> task_group
        globals()[dag_id] = dag
    except Exception:
        LOG.exception("Skip invalid DataWorks job DAG definition: %s", dag_def)
