...
CREATE TABLE IF NOT EXISTS data_work_task (
    ...
    status VARCHAR(20) DEFAULT 'DRAFT' COMMENT '任务状态：DRAFT（草稿）/ ONLINE（已上线）/ OFFLINE（已下线）',
    created_by VARCHAR(100) COMMENT '创建人',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted_at DATETIME DEFAULT NULL COMMENT '删除时间，逻辑删除标记',
    INDEX idx_folder_id (folder_id) COMMENT '文件夹索引',
    INDEX idx_name (name) COMMENT '任务名称索引，用于模糊搜索',
    INDEX idx_engine_type (engine_type) COMMENT '引擎类型索引，用于筛选'
) COMMENT = '数据加工任务表（旧版），存储SparkSQL/FlinkSQL任务定义';

CREATE TABLE IF NOT EXISTS schedule_config (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '调度配置ID，主键，自增',
    task_id BIGINT NOT NULL UNIQUE COMMENT '关联的任务ID',
    cron_expression VARCHAR(100) NOT NULL COMMENT 'Cron表达式，定义调度周期',
    enabled BOOLEAN DEFAULT FALSE COMMENT '是否启用调度：true（启用）/ false（停用）',
    next_execute_time DATETIME COMMENT '下次执行时间',
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    INDEX idx_task_id (task_id) COMMENT '任务ID索引'
) COMMENT = '调度配置表，存储数据加工任务的Cron调度参数';

CREATE TABLE IF NOT EXISTS execution_record (
    id BIGINT PRIMARY KEY AUTO_INCREMENT COMMENT '执行记录ID，主键，自增',
    task_id BIGINT NOT NULL COMMENT '关联的任务ID',
    task_name VARCHAR(200) COMMENT '任务名称（快照，记录执行时的名称）',
    ...
    INDEX idx_task_id (task_id) COMMENT '任务ID索引，用于按任务查询历史',
